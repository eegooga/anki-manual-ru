...
`preset:"Default"`\
карточки во всех колодах, использующих набор настроек «По умолчанию» (`Default`).
...
обрабатывает последующую часть как [сырой ввод](#raw-input), поэтому учитывайте
...
`is:review`\
карточки повторения (как срочные, так и несрочные), а также карточки после срыва.
...
`prop:lapses>3`\
карточки, которые уходили в срыв более трёх раз.

`prop:ease!=2.5`\
карточки, которые легче или сложнее стандартного коэффициента лёгкости.
...
`prop:r<0.9`\
карточки с извлекаемостью ниже 0.9.
...
`rated:1:2`\
карточки, которым сегодня поставили оценку «Трудно» (2).

`rated:7:1`\
карточки, которым за последние 7 дней поставили оценку «Снова» (1).

`rated:31:4`\
карточки, которым за последние 31 день поставили оценку «Легко» (4).
...
ID заметок и карточек можно увидеть в окне обзора, в диалоге [сведений о карточке](stats.md).
Эти запросы также полезны при разработке дополнений и работе с БД.
