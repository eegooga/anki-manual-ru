...
кнопка `Custom Study` («Выборочное обучение»),
...
`Custom Study Session` («Сеанс выборочного обучения»),
...
или все карточки;
после нажатия `Choose Tags` («Выбрать теги»)
...
- Кнопка `Empty` («Очистить») в обзоре обучения
...
в меню `Tools` («Инструменты»).

Когда вы нажимаете `Build` («Собрать»),
...
используйте кнопку `Rebuild` («Пересобрать»)
...
Опция `cards selected by` («карточки выбраны по»)
управляет порядком,
