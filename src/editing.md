...
[главном окне](studying.md#decks) — откроется окно добавления заметок (`Add Notes`).

![Окно добавления](media/add_screen.png)
...
карточек во время повторения; для этого см. [шаблоны](templates/intro.md).
...
- **Editing Font** позволяет настроить шрифт и размер текста при
  ...
  [шаблоны](templates/intro.md). Однако при включённой функции «ввод ответа»
  (`type in the answer`) введённый текст будет использовать размер шрифта, заданный здесь.
  (О смене самого шрифта при вводе ответа см. [проверка вашего ответа](templates/fields.md#checking-your-answer).)

- **Sort by this field…** сообщает Anki, что это поле нужно показывать в
  столбце поля сортировки (`Sort Field`) окна обзора. По нему можно сортировать карточки.
...
- **Reverse text direction** полезно для языков с письмом справа налево (RTL),
  ...
  [шаблон](templates/styling.md#text-direction).
...
- **Collapse by default**: поля можно сворачивать/разворачивать.
  Анимацию можно отключить в [настройках](preferences.md).
...
карточки. Подробнее см. раздел [шаблоны](templates/intro.md).
...
[Колоды](getting-started.md#decks) предназначены для деления материала на
...
Теги можно добавлять в окне редактирования (`Edit`) и в [обзоре](browsing.md); там же их можно
...
С помощью [режима выборочного обучения и фильтрованных колод](filtered-decks.md) можно создавать
...
![Иконки редактора](media/editor_icons.png)

Слева сверху находятся две кнопки, открывающие окна
[поля](editing.md#customizing-fields) и [карточки](templates/intro.md).
