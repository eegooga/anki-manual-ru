...
- [Общие колоды и основы повторения (Shared Decks and Review Basics)](http://www.youtube.com/watch?v=QS2G-k2hQyg&yt:cc=on)

- [Синхронизация (Syncing)](https://www.youtube.com/watch?v=YkiM4DPzSVc&list=PLGgmaKOIHykFoomqkBJAyGiDQ2kyiuTao&yt:cc=on)

- [Изменение порядка карточек (Switching Card Order)](http://www.youtube.com/watch?v=DnbKwHEQ1mA&yt:cc=on)

- [Оформление карточек (Styling Cards)](http://www.youtube.com/watch?v=F1j1Zx0mXME&yt:cc=on)

- [Ввод ответа с клавиатуры (Typing in the Answer)](http://www.youtube.com/watch?v=5tYObQ3ocrw&yt:cc=on)
...
- **Новая:** Карточки, которые вы скачали или создали,
  но ещё ни разу не изучали.

- **Изучаемая:** Карточки, которые вы впервые увидели недавно
  и всё ещё изучаете.

- **Повторяемая:** Карточки, которые вы уже выучили.
  Они будут показаны снова после истечения задержки (интервала).
  Есть два типа повторяемых карточек:
  - **Молодая:** Карточка с интервалом менее 21 дня.
  - **Зрелая:** Карточка с интервалом 21 день и более.

- **Переизучаемая:** Карточки,
...
Если выбрать `Hanzi`,
будут показаны только карточки из колоды Hanzi;
если выбрать `Chinese`,
будут показаны все карточки из колоды Chinese,
включая карточки Hanzi.
...
часто называют подколодами,
а колоды верхнего уровня — родительскими колодами.

Anki начинается с колоды `По умолчанию` (`Default`);
...
Например, `Моя колода 10` будет перед `Моя колода 9`,
так как 1 идёт перед 9.
Если вы хотите, чтобы `Моя колода 9` шла раньше,
можно переименовать её в `Моя колода 09`,
которая идёт раньше `Моя колода 10`.

Колоды лучше использовать для широких категорий карточек,
а не для узких тем вроде «глаголы по теме “еда”» или «урок 1».
...
и узнали, что слово _bonjour_ означает «здравствуйте»,
вы можете захотеть сделать одну карточку,
которая показывает `bonjour` и просит вспомнить «здравствуйте»,
и вторую карточку,
которая показывает «здравствуйте» и просит вспомнить `bonjour`.
...
- **Базовый (`Basic`)**
  Имеет поля `Front` («лицевая сторона») и `Back` («оборотная сторона»)
  и создаёт одну карточку.
  Текст, введённый в `Front`, показывается на лицевой стороне,
  а текст из `Back` — на оборотной.

- **Базовый с обратной карточкой (`Basic (and reversed card)`)**\
  Как базовый тип, но создаёт две карточки
  для введённого текста:
  с лицевой стороны на оборотную и с оборотной на лицевую.

- **Базовый с необязательной обратной карточкой (`Basic (optional reversed card)`)**\
  Как базовый тип, но имеет третье поле `Add Reverse`
  («добавить обратную»).
  Если в это поле ввести любой текст,
  дополнительно будет создана обратная карточка
  (с оборотной стороны на лицевую).
  Подробности см. в разделе [Карточки и шаблоны](templates/intro.md).

- **Базовый с вводом ответа (`Basic (type in the answer)`)**\
  По сути это базовый тип,
...
- **Пропуски в тексте (`Cloze Deletion`)**
  Тип заметки, позволяющий выделить текст
  и превратить его в заметку с пропусками
  (например, `Человек высадился на Луну в [....] году`
  → `Человек высадился на Луну в 1969 году`).
  Подробности см. в разделе [пропуски в тексте (cloze deletion)](editing.md#cloze-deletion).

- **Скрытие изображения (`Image Occlusion`)**
  Похоже на тип заметки с пропусками,
...
Когда вы добавляете заметки через окно добавления,
...
3. Дважды щёлкните скачанный пакет, чтобы импортировать его в Anki,
   или выберите **Файл > Импорт**.

Примечание: в данный момент нельзя добавить общие колоды
напрямую в аккаунт AnkiWeb.
Сначала нужно импортировать их в настольное приложение,
